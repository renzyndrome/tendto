...
3. **All IDs are client-generated UUIDs; every synced row carries `updated_at`.** Writes must be
   idempotent (safe to replay from the upload queue).
...
- **apps/api**: FastAPI (async), SQLAlchemy 2 (async) + Alembic, Pydantic v2, Postgres
  (Supabase in cloud, plain Postgres in docker for dev), Clerk JWT verification.
- **infra**: docker compose (postgres + powersync service), `infra/powersync/sync-rules.yaml`.

## Commands

- `make db` / `make db-down` — start/stop dev Postgres (+ PowerSync service)
- `make api` — run FastAPI dev server (uvicorn, :8000)
- `make web` — run Vite dev server (:5173)
