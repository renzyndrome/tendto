...
- [Page links & backlinks](page-links.md) — `[[` is wired to a single `[` (BlockNote cannot match a 2-char trigger mid-line); React events never reach an inline node view; `useQuery` returns `[]` before it loads; `page_links` is derived and trigger-built, so neither it nor FTS can be watched with `useQuery`
