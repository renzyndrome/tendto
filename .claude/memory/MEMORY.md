...
- [Page links](page-links.md) — `[[` is wired to a single `[` (BlockNote cannot match a 2-char trigger mid-line); React events never reach an inline node view; `useQuery` returns `[]` before it loads
